...
describe('session de leçon', () => {
  it('présente chaque mot avant de le tester au niveau 0', () => {
    const session = buildLessonSession('u1-l1', LESSON, 0)
    ...
    for (const intro of intros) {
      ...
      const tested = session.findIndex(
        (exercise, index) =>
          index > introIndex && exercise.kind !== 'intro' && vocabOf(exercise).some((v) => v.id === intro.vocab.id),
      )
  ...
  it('ne présente plus rien aux niveaux suivants', () => {
    expect(kinds(buildLessonSession('u1-l1', LESSON, 1))).not.toContain('intro')
    expect(kinds(buildLessonSession('u1-l1', LESSON, 2))).not.toContain('intro')
  })

  it('passe de la reconnaissance à la production selon le niveau', () => {
    const level0 = kinds(buildLessonSession('u1-l1', LESSON, 0))
    const level2 = kinds(buildLessonSession('u1-l1', LESSON, 2))
  ...
  it('propose une banque de mots au niveau 1, la retire au niveau 2', () => {
    const guided = buildLessonSession('u1-l1', LESSON, 1).filter((e) => e.kind === 'cloze')
    const free = buildLessonSession('u1-l1', LESSON, 2).filter((e) => e.kind === 'cloze')
  ...
  it('est déterministe à graine égale, différente d’un niveau à l’autre', () => {
    expect(buildLessonSession('u1-l1', LESSON, 1)).toEqual(buildLessonSession('u1-l1', LESSON, 1))
    expect(buildLessonSession('u1-l1', LESSON, 1)).not.toEqual(buildLessonSession('u1-l1', LESSON, 2))
  })

  it('n’enchaîne pas deux exercices sur le même mot', () => {
    for (const level of [1, 2]) {
      const session = buildLessonSession('u1-l1', LESSON, level)
      for (let i = 1; i < session.length; i++) {
        const previous = new Set(vocabOf(session[i - 1]).map((v) => v.id))
        const repeated = vocabOf(session[i]).filter((v) => previous.has(v.id))
  ...
  it('ignore les mots sans phrase d’exemple exploitable', () => {
    ...
    const clozes = buildLessonSession('x', vocab, 2).filter((e) => e.kind === 'cloze')
  ...
  it('n’ajoute pas de manche d’association sous quatre mots', () => {
    ...
    expect(kinds(buildLessonSession('x', tiny, 0))).not.toContain('match')
  })
})

describe('session de révision', () => {
  const entries = (states: Partial<CardState>[]) =>
  ...
  it('teste chaque carte au moins une fois', () => {
    ...
    const covered = new Set(session.flatMap((exercise) => vocabOf(exercise).map((v) => v.id)))
