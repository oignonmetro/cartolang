#!/usr/bin/env node
// forked from design-sync package-capture.mjs — settle() only waits on
// document.fonts.ready + image decode, never on JS-driven entrance
// animations (VocabIntro's mount transition, framer-motion's `initial` →
