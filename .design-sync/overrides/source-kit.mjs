// forked from design-sync lib/source-kit.mjs — cartolang has no dist/ (it's
// a private app, not a published package), so the synth-entry barrel is
// built straight from every matching file under src/components/. One file
// there, UpdatePrompt.tsx, imports `virtual:pwa-register/react`, a Vite
// build-time virtual module with no on-disk resolution — esbuild fails the
// whole bundle on it, and cfg.componentSrcMap only filters the *component
// list*, not the barrel's file set, so excluding it there doesn't help. This
// fork adds `cfg.srcExclude` (bare filenames, matched against the file's
// basename) filtered into the barrel's file list itself, the one point
// upstream has no override for.
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // ASSUMPTION: files whose basename is listed in cfg.srcExclude never enter
    // the synth-entry barrel — they're excluded from the bundle outright, not
    // just from the discovered component list. Override: cfg.srcExclude.
    const excluded = new Set(cfg.srcExclude ?? []);
    const comps = srcFiles.filter(
      (p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !excluded.has(basename(p)),
    );
